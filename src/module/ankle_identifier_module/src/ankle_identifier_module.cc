...
void AnkleIdentifierModule::MainLoop() {
  ...
  while (run_flag_.load()) {
    ...
    if (!baseline_captured_.load()) {
      static int baseline_loop_count = 0;
      if (baseline_loop_count++ % 1000 == 0) {
        AIMRT_INFO("Baseline capture loop iteration {}", baseline_loop_count);
      }
...
void AnkleIdentifierModule::PublishKickCommand() {
  my_ros2_proto::msg::JointCommand cmd;
  cmd.name.reserve(startup_target_positions_.size());
  cmd.position.reserve(startup_target_positions_.size());
  cmd.velocity.reserve(startup_target_positions_.size());
  cmd.effort.reserve(startup_target_positions_.size());
  cmd.stiffness.reserve(startup_target_positions_.size());
  cmd.damping.reserve(startup_target_positions_.size());
  for (const auto& [name, pos] : startup_target_positions_) {
    cmd.name.push_back(name);
    cmd.position.push_back(pos);
    cmd.velocity.push_back(0.0);
    cmd.effort.push_back(0.0);
    const auto [kp, kd] = GetHoldGains(name);
    cmd.stiffness.push_back(kp);
    cmd.damping.push_back(kd);
  }
  aimrt::channel::Publish<my_ros2_proto::msg::JointCommand>(joint_cmd_pub_, cmd);
}
