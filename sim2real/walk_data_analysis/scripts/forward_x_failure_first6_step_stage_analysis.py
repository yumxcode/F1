...
OUT_DIR = os.path.join(ANALYSIS_DIR, "table", "forward_x_failure_first6")
RESULT_DIR = os.path.join(ANALYSIS_DIR, "reports")
...
ROUND3A = load_module(
    "landing_window_analysis",
    os.path.join(SCRIPT_DIR, "landing_window_analysis.py"),
)
        ...
        for window_name in ("swing", "touchdown"):
            ...
            for side in ("left", "right"):
                for axis in ANKLE_AXES:
                    ...
                    lag_ms_val = lag_samples * dt_sec * 1000.0 if not math.isnan(lag_samples) else math.nan
                    target_range = target_metrics["range"]
                    joint_range = joint_metrics["range"]
                    joint_vel_rms = joint_metrics["vel_rms"]
                    target_vel_rms = target_metrics["vel_rms"]
                    aligned_err_rms = rms(aligned_err)

                    # M5: velocity self-excitation ratio
                    if (not math.isnan(target_vel_rms) and target_vel_rms > 1e-6
                            and not math.isnan(joint_vel_rms)):
                        vel_ratio = joint_vel_rms / target_vel_rms
                    else:
                        vel_ratio = math.nan

                    # range_gain_phase: joint amplitude / target amplitude (M4 amplitude component)
                    if (not math.isnan(target_range) and target_range >= MIN_GAIN_TARGET_AMP_RAD
                            and not math.isnan(joint_range)):
                        range_gain = joint_range / target_range
                    else:
                        range_gain = math.nan

                    # M11: frequency-gain cross-statistics — target dominant freq, modal candidate, gap
                    target_dominant_freq = target_metrics["dominant_freq_hz"]
                    f_modal_candidate = err_metrics["dominant_freq_hz"]
                    if (not math.isnan(target_dominant_freq) and not math.isnan(f_modal_candidate)):
                        freq_gap_hz_val = abs(target_dominant_freq - f_modal_candidate)
                    else:
                        freq_gap_hz_val = math.nan

                    # M8: joint_lock_flag — joint frozen while target moving
                    joint_lock = (
                        (not math.isnan(joint_range) and joint_range < 0.02)
                        and (not math.isnan(target_range) and target_range > 0.10)
                        and (not math.isnan(aligned_err_rms) and aligned_err_rms > 0.25)
                    )
                    ...
                    delay_rows.append(
                        {
                            ...
                            "lag_ms": lag_ms_val,
                            "corr": corr,
                            # ── unified field name aliases (plan naming convention) ──
                            "tracking_lag_ms": lag_ms_val,
                            "xcorr_coeff": corr,
                            "range_gain_phase": range_gain,
                            # ── M5: velocity self-excitation ──
                            "joint_vel_rms_radps": joint_vel_rms,
                            "target_vel_rms_radps": target_vel_rms,
                            "velocity_self_excitation_ratio": vel_ratio,
                            # ── M8: joint lock ──
                            "joint_lock_flag": int(joint_lock),
                            "raw_sample_count": len(window_rows),
                            "aligned_sample_count": len(aligned_joint),
                            "target_range_rad": target_range,
                            "joint_range_rad": joint_range,
                            ...
                            "target_dominant_freq_hz": target_dominant_freq,
                            "f_modal_candidate_hz": f_modal_candidate,
                            "freq_gap_hz": freq_gap_hz_val,
                            # ── M11 alias ──
                            "amplitude_gain": range_gain,
                            "aligned_tracking_err_rms_rad": aligned_err_rms,
...
def _step_risk_score(rows_for_step):
    """
    Compute a single risk score for a step from its delay_rows.
    Uses swing-window rows only (most informative), averaged over all joints.
    """
    swing = [r for r in rows_for_step if r.get("window") == "swing"]
    if not swing:
        swing = rows_for_step

    def nanmean_key(key):
        vals = [r[key] for r in swing if not math.isnan(r.get(key, math.nan))]
        return mean(vals) if vals else math.nan

    err_rms = nanmean_key("aligned_tracking_err_rms_rad")
    vel_ratio = nanmean_key("velocity_self_excitation_ratio")
    lock = max((r.get("joint_lock_flag", 0) for r in swing), default=0)
    gain = nanmean_key("range_gain_phase")

    score = 0.0
    if not math.isnan(err_rms):
        score += min(err_rms / 0.15, 1.0) * 30.0   # up to 30 pts
    if not math.isnan(vel_ratio):
        score += min(max(vel_ratio - 1.0, 0.0) / 1.0, 1.0) * 30.0  # up to 30 pts
    if lock:
        score += 30.0                                 # binary: 30 pts
    if not math.isnan(gain):
        score += min(max(gain - 1.0, 0.0) / 1.0, 1.0) * 10.0  # up to 10 pts
    return round(score, 2)


def add_accumulation_metrics(delay_rows):
    """
    Post-process delay_rows to add per-step M9 step_accumulation_score,
    risk_tags, and overall_status per case.

    Modifies delay_rows in-place.
    """
    # Group by (dataset, case_label, step_index)
    step_groups = defaultdict(list)
    for row in delay_rows:
        key = (row["dataset"], row["case_label"], row["step_index"])
        step_groups[key].append(row)

    # Compute risk score per step, then accumulation score (monotonic sum)
    case_groups = defaultdict(list)
    for (dataset, case_label, step_idx), step_rows in sorted(step_groups.items()):
        case_groups[(dataset, case_label)].append((step_idx, step_rows))

    for (dataset, case_label), steps in case_groups.items():
        steps_sorted = sorted(steps, key=lambda x: x[0])
        scores = [_step_risk_score(rows) for _, rows in steps_sorted]

        # M9: step_accumulation_score = count of steps where risk >= previous peak
        peak = 0.0
        accum = 0
        accum_per_step = []
        for s in scores:
            if s >= peak:
                peak = s
                accum += 1
            accum_per_step.append(accum)

        # Build risk tags per case (aggregate over steps)
        for step_pos, (step_idx, step_rows) in enumerate(steps_sorted):
            step_score = scores[step_pos]
            step_accum = accum_per_step[step_pos]

            # Risk tags for this step
            tags = []
            swing_rows = [r for r in step_rows if r.get("window") == "swing"]
            all_rows = swing_rows if swing_rows else step_rows
            err_rms_vals = [r.get("aligned_tracking_err_rms_rad", math.nan) for r in all_rows]
            vel_ratios = [r.get("velocity_self_excitation_ratio", math.nan) for r in all_rows]
            locks = [r.get("joint_lock_flag", 0) for r in all_rows]
            gains = [r.get("range_gain_phase", math.nan) for r in all_rows]

            err_mean = mean(err_rms_vals)
            vel_mean = mean(vel_ratios)

            if not math.isnan(err_mean) and err_mean > 0.10:
                tags.append("high_tracking_err")
            if not math.isnan(vel_mean) and vel_mean > 1.5:
                tags.append("velocity_self_excitation")
            if any(l > 0 for l in locks):
                tags.append("joint_lock")
            if not math.isnan(mean(gains)) and mean(gains) > 1.5:
                tags.append("amplitude_overshoot")
            if step_score >= 50:
                tags.append("risk")
            elif step_score >= 25:
                tags.append("warn")

            risk_tag_str = ";".join(tags) if tags else "ok"

            # overall_status per case: fail if any step has risk, warn if any warn
            # (computed after all steps processed — injected below)
            for row in step_rows:
                row["step_risk_score"] = step_score
                row["step_accumulation_score"] = step_accum   # M9
                row["risk_tags"] = risk_tag_str

        # Compute overall_status for this case
        all_tags_flat = []
        for _, step_rows in steps_sorted:
            all_tags_flat.extend(step_rows[0].get("risk_tags", "ok").split(";") if step_rows else [])
        if "risk" in all_tags_flat:
            status = "fail"
        elif "warn" in all_tags_flat:
            status = "warn"
        else:
            status = "ok"
        for _, step_rows in steps_sorted:
...
def build_markdown(path, delay_rows, cycle_rows, touchdown_rows, profile_rows):
    ...
    with open(path, "w", encoding="utf-8") as handle:
        ...
        handle.write("## 3b. M5/M8/M9 Risk Summary\n\n")
        handle.write("| dataset | case | step | side | axis | window | tracking_lag_ms | xcorr_coeff | range_gain | vel_ratio | joint_lock | step_risk | accum_score | risk_tags | overall |\n")
        handle.write("|---|---|---:|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|---|\n")
        for key in sorted(delay_by_group.keys()):
            dataset, case_label, step_index, window_name = key
            if window_name != "swing":
                continue
            rows_g = sorted(delay_by_group[key], key=lambda item: (item["side"], item["axis"]))
            for row in rows_g:
                handle.write(
                    f"| {row['dataset']} | {row['case_label']} | {row['step_index']} "
                    f"| {row['side']} | {row['axis']} | {window_name} "
                    f"| {fmt(row.get('tracking_lag_ms', math.nan), 1)} "
                    f"| {fmt(row.get('xcorr_coeff', math.nan), 3)} "
                    f"| {fmt(row.get('range_gain_phase', math.nan), 3)} "
                    f"| {fmt(row.get('velocity_self_excitation_ratio', math.nan), 3)} "
                    f"| {row.get('joint_lock_flag', 0)} "
                    f"| {fmt(row.get('step_risk_score', math.nan), 1)} "
                    f"| {row.get('step_accumulation_score', '')} "
                    f"| {row.get('risk_tags', '')} "
                    f"| {row.get('overall_status', '')} |\n"
                )
        handle.write("\n")
...
# ═══════════════════════════════════════════════════════════════════════════════
# M12: Overall FFT frequency analysis (per-file, per-joint, per-phase)
# ═══════════════════════════════════════════════════════════════════════════════
...
def fft_spectrum(signal, fs=100.0):
    n = len(signal)
    amp = 2.0 * np.abs(np.fft.rfft(signal * np.hanning(n))) / n
    freq = np.fft.rfftfreq(n, d=1.0 / fs)
    return freq, amp


def top_peaks(freq, amp, fmin=FFT_FMIN_HZ, fmax=FFT_FMAX_HZ, n_peaks=3):
    mask = (freq >= fmin) & (freq <= fmax)
    if not np.any(mask):
        return []
    fsub = freq[mask]
    asub = amp[mask]
    idx = np.argsort(asub)[::-1][:n_peaks]
    return [(float(fsub[i]), float(asub[i])) for i in idx if asub[i] > 1e-6]


def band_energy_ratio(freq, amp, flo, fhi):
    mask = (freq >= flo) & (freq <= fhi)
    total = np.sum(amp ** 2)
    if total < 1e-12:
        return 0.0
    return float(np.sum(amp[mask] ** 2) / total)


def compute_fft_rows(cases, base_dir):
    """Per-file × per-joint × per-phase FFT analysis."""
    rows = []
    for dataset, case_label, rel_path in cases:
        path = os.path.join(base_dir, rel_path)
        try:
            df = pd.read_csv(path)
        except Exception:
            continue
        t_ns = df["timestamp_ns"].values.astype(np.float64)
        t = (t_ns - t_ns[0]) / 1e9
        fs = 1.0 / np.nanmedian(np.diff(t)) if len(t) > 2 else 100.0

        for side in ["left", "right"]:
            contact_col = f"{side}_contact"
            contact = df[contact_col].values.astype(float) > 0.5 if contact_col in df.columns else np.zeros(len(t), dtype=bool)

            for axis in ["pitch", "roll"]:
                pos_col = f"pos_{side}_ankle_{axis}_joint"
                des_col = f"pos_des_raw_{side}_ankle_{axis}_joint"
                if pos_col not in df.columns or des_col not in df.columns:
                    continue
                pos = df[pos_col].values.astype(float)
                ...
                for phase_name, phase_mask in [
                    ...
                    p_pos = pos[phase_mask]
                    p_des = des[phase_mask]
                    p_err = err[phase_mask]
                    n_samples = len(p_pos)
                    ...
                    row = {
                        "dataset": dataset, "case_label": case_label,
                        "side": side, "axis": axis, "phase": phase_name,
                        "n_samples": n_samples, "duration_sec": round(duration, 2),
                    }
                    for sig_name, sig in [("pos", p_pos), ("des", p_des), ("err", p_err)]:
                        sig_c = sig[np.isfinite(sig)]
                        if len(sig_c) < 32:
                            continue
                        freq, amp = fft_spectrum(sig_c, fs)
                        peaks = top_peaks(freq, amp)
                        for ipk, (pf, pa) in enumerate(peaks[:3], 1):
                            row[f"{sig_name}_peak{ipk}_hz"] = round(pf, 3)
                            row[f"{sig_name}_peak{ipk}_amp"] = round(pa, 5)
                        for band_name, flo, fhi in FFT_BANDS:
                            row[f"{sig_name}_energy_{band_name}"] = round(
                                band_energy_ratio(freq, amp, flo, fhi), 4
                            )
                        row[f"{sig_name}_osc_rad"] = round(float(np.std(sig_c)), 5)
                    rows.append(row)
    return rows


def build_fft_summary_rows(fft_rows):
    """Aggregate FFT rows by dataset × case_label × axis × phase."""
    groups = defaultdict(list)
    for r in fft_rows:
        groups[(r["dataset"], r["case_label"], r["axis"], r["phase"])].append(r)
    out = []
    for (dataset, case_label, axis, phase), items in groups.items():
        row = {
            "dataset": dataset, "case_label": case_label,
            "axis": axis, "phase": phase,
            "n_joints": len(items),
            "total_duration_sec": sum(it["duration_sec"] for it in items),
        }
        for sig_name in ["pos", "err"]:
            for ipk in [1, 2]:
                hz_vals = [it.get(f"{sig_name}_peak{ipk}_hz") for it in items
                          if not math.isnan(it.get(f"{sig_name}_peak{ipk}_hz", math.nan))]
                if hz_vals:
                    row[f"{sig_name}_peak{ipk}_hz_med"] = round(median(hz_vals), 2)
            for band_name, _, _ in FFT_BANDS:
                e_vals = [it.get(f"{sig_name}_energy_{band_name}") for it in items
                         if not math.isnan(it.get(f"{sig_name}_energy_{band_name}", math.nan))]
                if e_vals:
                    row[f"{sig_name}_energy_{band_name}_med"] = round(median(e_vals), 3)
            osc_vals = [it.get(f"{sig_name}_osc_rad") for it in items
                       if not math.isnan(it.get(f"{sig_name}_osc_rad", math.nan))]
            if osc_vals:
                row[f"{sig_name}_osc_rad_med"] = round(median(osc_vals), 4)
        out.append(row)
    return out
...
def main():
    ...
    # M9 + risk_tags + overall_status post-processing
    add_accumulation_metrics(delay_rows)
    ...
    fft_detail_csv  = os.path.join(OUT_DIR, "forward_x_failure_first6_fft_detail.csv")
    fft_summary_csv = os.path.join(OUT_DIR, "forward_x_failure_first6_fft_summary.csv")
    result_md = os.path.join(RESULT_DIR, "前6步故障阶段分析报告.md")
    ...
    # M12: Overall FFT analysis
    all_cases = [(ds, cl, rp) for ds, cl, rp in REAL_CASES + SIM_CASES]
    fft_rows = compute_fft_rows(all_cases, BASE_DIR)
    fft_summary = build_fft_summary_rows(fft_rows)
    write_csv(fft_detail_csv, fft_rows)
    write_csv(fft_summary_csv, fft_summary)
    ...
    print(profile_csv)
    print(hist_csv)
    print(fft_detail_csv)
    print(fft_summary_csv)
    print(result_md)
