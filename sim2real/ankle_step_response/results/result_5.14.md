...
- `Round 2B`: 触地退化测量 ✅
- `Round 2C`: 定向修复（提升 kp / kd，消除 timing 退化与振荡分裂）← **当前阶段**（执行单：[03_round_02c_contact_degradation_fix.md](/Users/yumx/code/X1/agibot_x1_infer/sim2real/ankle_step_response/plans/round_02/03_round_02c_contact_degradation_fix.md)）
...
| `right_ankle_roll_joint` | `50 / 0.8` | `best_air_candidate` | 当前唯一稳定进入 `well_damped_tracking` 的点；`overshoot_ratio = 0`，`zero_crossing_count = 0`，3 次重复完全一致；时间响应满足 动态响应 预算 |
...
>
> 执行单：[02_round_02b_ground_degradation_test.md](/Users/yumx/code/X1/agibot_x1_infer/sim2real/ankle_step_response/plans/round_02/02_round_02b_ground_degradation_test.md)（含测试程序和通过/失败判据）
>
...
| `30/0.5` | `air` | `0.879 ± 0.131` | `0.928 ± 0.133` | `0.928 ± 0.133` | `0.098 ± 0.013` | `well_damped_tracking / undershoot_soft / single_overshoot` | `Round 2D` 复测。3 次明显分裂：`0.876 / 0.750 / 1.012`；其中 1 次 `rise_time_status = too_slow_for_dynamic_response`，不再是稳定可用点 | `pending_recheck_split` |
...
- **air 选择原因**：
  ...
  - 时间响应满足 动态响应 预算：`rise_time_sec ≈ 0.030`，`peak_time_sec ≈ 0.049`
...
- **Round 2C 方向（left roll 最复杂）**：
  ...
  - **可接受性评估**：动态响应 中 roll 方向实际命令幅值若主要在 `0.030~0.050 rad`，`tracking ≈ 0.757` 是否已足够——最终需要 后续验证 动态验证
...
- **Round 2C 方向修正**：
  ...
  - 核心问题是：**动态响应 期间 right roll 的实际命令幅值是否能持续超过接触阈值？**
  - 若 RL 动态响应 策略对 right roll 的命令幅值主要集中在 `≥ 0.03~0.05 rad`，则 `kp=50` 可能直接可用
  - 若 动态响应 中 right roll 主要做小幅修正（`< 0.02 rad`），则仍需要提升 kp
  - **建议下一步**：先补测 `0.030 rad` 和 `0.050 rad` 的触地阶跃，找到接触阈值的临界幅值，再决定是否提升 kp
- **保留问题**：
  - 小幅值触地跟踪不足，动态任务中小幅修正能力存疑
  - 大幅值触地表现良好，结构上没有根本性问题
  - `final_tracking_ratio ≈ 0.012`（0.100 rad 工况），active 窗口结束后关节仍会回弹（接触下无持续保持力矩），动态执行时持续命令可覆盖此现象
...
- **Round 2B 完整结论（两条口径）**：
  ...
  - `0.100 rad ground`：tracking 恢复（`0.815`），但出现**时序退化**——iter2/3 的 `peak_time = 0.285~0.291 sec`（`unusable_for_dynamic_response`），超出 动态响应 预算近一倍
- **与 right roll 的本质区别**：
...
- **`peak_time` 分裂机制分析**：
  ...
  - iter2/3 `peak_time = 0.285~0.291 sec`（unusable）：多次激励后接触面状态改变（压实/变形），pitch 方向等效接触刚度升高，`kp=40` 已无法在 动态响应 时序内驱动关节到达目标
...
当前 Round 2B 的所有 ground 测试均在**双脚同时触地、机器人静止站立**工况下进行。这比实际动态任务工况更保守：

| 测试工况 | 接触状态 | 约束强度 | 对应场景 |
|---|---|---|---|
| Round 2B（当前）| 双脚触地，静止 | **最高**：双侧约束，全重压脚，无惯性辅助 | 静态辨识 |
| 实际动态任务（目标） | 主要单脚支撑，动态 | **较低**：单侧约束，动态接触，惯性可辅助 | 动态任务运行 |
...
- 双脚静态接触下的"接触阈值"在单脚动态动态任务中将因单侧负载减半和惯性助力而显著降低

**实践意义：Round 2B 数据揭示的退化量，是触地工况下的上界估计。实际动态任务中的退化将明显小于 Round 2B 测试结果。**

### 四关节 Round 2B 完整汇总

| 关节 | `kp/kd` | `0.015rad ground` | `0.100rad ground` | 退化类型 | Round 2C 优先动作 |
|---|---|---|---|---|---|
| `right_ankle_roll_joint` | `50/0.8` | `0.415`，severe | `0.832`，well_damped ✓ | 接触阈值型，完全恢复 | 补测中间幅值（0.030/0.050 rad），评估 动态响应 可用性 |
...
1. **right pitch**（最高）：timing 退化是动态执行时序的直接威胁，必须修复后才能安全进入 后续验证
2. **left pitch**（高）：振荡分裂在 kd=0.8 下出现，kd=1.0 的 ground 验证是一次测试的事
3. **right roll**（中）：技术上已可接受，补测中间幅值确认阈值后可直接进 后续验证
4. **left roll**（观察）：先排查硬件，但不应作为 后续验证 的硬性阻塞条件——动态响应 中的单脚支撑可能使 0.757 的跟踪率已足够
