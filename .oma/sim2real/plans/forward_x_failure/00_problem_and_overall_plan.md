...
| 04 | [04_tracking_lag_repair.md](/Users/yumx/code/X1/agibot_x1_infer/.oma/sim2real/plans/forward_x_failure/04_tracking_lag_repair.md:1) | 基于 step 试验和 Round 3 跟踪滞后样本，专项验证并修复执行链不足 | ready to execute |
...
3. `03` 已经把根因拆成 `command_not_flat / tracking_lag / coupled_geometry`，当前先执行 `04_tracking_lag_repair`。
4. 若 `04` 证明执行链不足，优先在部署侧修复踝执行能力，不盲目全局扫参数。
5. 若 `03`/`04` 之后仍保留 `command_not_flat`，进入策略/观测/奖励设计反馈，不在部署侧硬调。
6. 若 `03`/`04` 之后仍保留 `coupled_geometry`，新建几何/映射专项，不继续把问题归因到单轴 kp/kd。
7. `foot_clearance_deficit` 与 `hip_knee_swing_timing_or_amplitude` 保留为二级问题，在踝落地姿态主因收敛后复判。
8. 只有当 `01`、`03`、`04` 的阻塞项关闭后，才允许执行 `02_low_speed_walk_validation_candidate`。
